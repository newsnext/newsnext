...
describe("Nitro cache database config", () => {
  afterEach(() => {
    delete process.env.NITRO_PRESET
    vi.resetModules()
  })

  it("configures Bun and local cache databases through Nitro", async () => {
    const config = await loadConfig()

    expect(config.experimental?.database).toBe(true)
    expect(config.database?.default?.connector).toBe("bun-sqlite")
    expect(config.database?.default?.options?.path).toMatch(/\/data\/cache\.db$/)
    ...
    expect(config.devDatabase?.default?.options?.path).toMatch(/\/data\/cache\.db$/)
    expect(config.devDatabase?.default?.options?.path).not.toContain("file:")
  })

  it("configures Cloudflare cache database through Nitro", async () => {
    ...
    expect(cloudflareConfig.database?.default?.options?.bindingName).toBe("CACHE_DB")
  })
})
