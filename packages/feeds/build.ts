...
const feedDescriptors = []
for (const [provider, providerDefinition] of Object.entries(providers)) {
  for (const [feedId, feed] of Object.entries((providerDefinition as any).feeds)) {
    const { loader, ...meta } = feed as any
    feedDescriptors.push({
      ...meta,
      id: meta.id || feedId,
      provider,
    })
...
export const feedDescriptors: FeedDescriptor[] = ${JSON.stringify(feedDescriptors, null, 2)}
`

await Bun.file(join(rootDir, "src", "metadata.ts")).write(metadataContent)
console.log("Generated src/metadata.ts with", feedDescriptors.length, "entries")
