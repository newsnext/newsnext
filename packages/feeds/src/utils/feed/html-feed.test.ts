...
import { $htmlFeedLoader } from "./html-feed"
...
describe("$htmlFeedLoader", () => {
  ...
  it("should parse simple html correctly", async () => {
    ...
    const results = await (feed as any).loader({})
  ...
  it("should handle params in url function", async () => {
    ...
    await (feed as any).loader({ page: 2 })
