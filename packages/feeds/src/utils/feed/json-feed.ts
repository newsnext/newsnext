...
import type { FeedParamSchemaMap, InferFeedParams, NewsItem } from "../../typings/feeds"
import { createFeedLoader } from "."
import { myFetch } from "../fetch"

export type FieldResolver<Item = any, Result = any> = string | ((item: Item) => Result)

export interface JsonFeedOptions<Item = any> {
  url: string
  /**
...
async function jsonFeedHandler<Item = any>(opts: JsonFeedOptions<Item>): Promise<NewsItem[]> {
...
export function $jsonFeedLoader<Item = any>(
  options: () => JsonFeedOptions<Item>,
): { loader: () => Promise<NewsItem[]> }
export function $jsonFeedLoader<P extends FeedParamSchemaMap, Item = any>(
  params: P,
  options: (params: InferFeedParams<P>) => JsonFeedOptions<Item>,
): { params: P, loader: (params: InferFeedParams<P>) => Promise<NewsItem[]> }
export function $jsonFeedLoader<Item = any>(
  ...args: any[]
): any {
  return (createFeedLoader<JsonFeedOptions<Item>>(jsonFeedHandler as any) as any)(...args)
}
