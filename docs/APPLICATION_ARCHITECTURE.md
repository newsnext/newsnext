...
before rendering: worker routing, catalog, and connection changes are emitted
through the background event center (`nativeIntegration.statusChanged`), which
invalidates the status query immediately, with the regular poll left as the
fallback for missed messages. Watching is best effort, so a daemon without it keeps the
...
The UI proxy service remains available while the optional CLI integration
