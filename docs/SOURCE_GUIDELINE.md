...
Run the JSON package build first so the source package consumes the latest JSON
registry. The extension build and development watcher run both builds in this
order automatically.
...
their respective build scripts. Metadata remains a lightweight build-time
catalog for synchronous UI discovery, while JSON provider definitions and
loaders are resolved lazily at runtime.

## Registry loading

`@newsnext/source/service` loads the source-only registry through an asynchronous
`SourceRegistryLoader`. The extension configures this loader during background
startup. Normal source loading performs no network request: it dynamically
imports the bundled `@newsnext/registry` and reads validated remote registries
