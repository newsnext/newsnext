...
Widget manifests, views, preset data contracts, and the custom document contract
live in the [Widget Guideline](WIDGET_GUIDELINE.md). Source-authored data reaches
them through the Widget's own `data.mjs` producer or `data.queries`, which use the
same query machinery described above; keep source configuration in this guide and
Widget presentation in the Widget Guideline.
