...
function EventCard({ event }: { event: CalendarEventItem }) {
  return (
    ...
    >
      <div className={cn("h-8 w-1 rounded-sm", event.barColor)}></div>
      <div className="flex-col items-center justify-center">
        <h4 className={cn("text-sm font-bold", event.color)}>{event.title}</h4>
        <p className={cn("whitespace-pre text-xs", event.dateColor)}>{event.time}</p>
      </div>
    </div>
  )
}
  ...
  return (
    ...
    >
      ...
      <div className="my-2 flex flex-1 flex-col gap-2">
        {dates.slice(0, maxEvents).map((event, index) => (
      ...
      {extraCount
        ? (
            <>
              ...
              {dates.slice(maxEvents, maxEvents + 3).map((_, index) => (
                <div
                  key={index}
                  style={{
                ...
                >
                  <div className="mt-px h-[2px] w-full rounded-full bg-gray-100" />
                </div>
              ))}
            </>
          )
