...
export function DeleteWidgetButton({ liveWidgetId }: { liveWidgetId: string }): React.JSX.Element {
  const { t } = useI18n()
  return (
    <DeleteCardButton
      ...
      onDelete={async () => {
        await actions.liveWidget.delete({ liveWidgetId })
      }}
    />
  )
}
  ...
  return (
    <CardBoardSelect
      ...
      onMove={async (targetBoardId) => {
        await actions.liveWidget.move({ liveWidgetId, boardId: targetBoardId })
      }}
    />
  )
}
