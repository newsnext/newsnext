...
describe("client source cache", () => {
  ...
  it("stores cache entries with an out-of-line key", async () => {
    const { readCachedClientSource, writeCachedClientSource } = await import("./client-source-cache")

    await writeCachedClientSource({
      id: "github:trending",
    ...
    const cached = await readCachedClientSource("github:trending:v1:{}", 60_000, 789)
