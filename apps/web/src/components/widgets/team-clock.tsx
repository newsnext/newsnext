...
export default function TeamClock({
  ...
  const handleUserSelect = (userName: string, timeDifference: string) => {
    if (selectedUser === userName) {
      ...
      setAngle(Number.parseInt(timeDifference) * 30)
    }
  }
  ...
  const handleUserHover = (userName: string | null, timeDifference: string | null) => {
    if (userName && timeDifference) {
      ...
      if (!selectedUser) {
        ...
        if (selectedUserData) {
          setAngle(Number.parseInt(selectedUserData.timeDifference) * 30)
        }
      }
    }
  }
...
function ListElement(props: ListElementProp) {
  ...
  const localTime = useMemo(() => {
    const hourDifference = Number.parseInt(props.timeDifference)
            ...
            <AnimatePresence>
              ...
              {(props.isSelected || isHovered) && (
                <motion.div
                  ...
                  style={{ color: Number.parseInt(props.timeDifference) < 0 ? "#EF4444" : "#10B981" }}
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  transition={{ duration: props.animationDuration }}
                >
                  {Number.parseInt(props.timeDifference) === 0
                    ? "+ 0 Hours"
